...
"""Validate Qwen3-Omni offline DPO LoRA with held-out DPO reward accuracy.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "use_audio_in_video=true is not supported yet."
        ),
    )
    parser.add_argument(
        "--log-level",
...
def main() -> None:
        ...
        for data_file, dataset, dataset_counts in datasets:
            ...
            for modality, indices in iter_modality_batches(dataset, args.batch_size):
                ...
                if args.log_every > 0 and batch_count % args.log_every == 0:
                    ...
                    logger.info(
                        "Progress: batches=%d/%d samples=%d/%d dpo_accuracy=%.4f dpo_margin=%.4f elapsed=%.1fs",
                        batch_count,
                        total_batches,
                        dpo_stats.total,
                        total_samples,
    ...
    logger.info(
        "Final dpo_accuracy=%.4f dpo_margin=%.4f",
        dpo_stats.accuracy,
        dpo_stats.mean_margin,
    )
