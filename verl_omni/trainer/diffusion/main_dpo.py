...
"""
Entrypoint for DPO / diffusion model training.
"""
...
def main(config):
    """Main entry point for DPO / diffusion model training.
...
def run_dpo(config, task_runner_class=None) -> None:
    """Initialize Ray and run the distributed DPO training process.
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # The shared diffusion trainer dispatches DPO-specific behavior from the dpo_trainer config.
