...
class PromptTxtRLDataset(RLHFDataset):
    ...
    - ``txt_default_data_source``: forwarded to ``data_source`` (default ``""``).
    ...
    def _make_example(self, line: str, index: int) -> dict[str, Any]:
        system = self.config.get("txt_system_prompt", None)
        neg_user = self.config.get("txt_negative_user_content", " ")
        user_only_neg = self.config.get("txt_user_only_negative_content", " ")
        ...
        reward_style = self.config.get("txt_reward_style", "rule")
        reward_gt = self.config.get("txt_reward_ground_truth", "")
        data_source = self.config.get("txt_default_data_source", "")
