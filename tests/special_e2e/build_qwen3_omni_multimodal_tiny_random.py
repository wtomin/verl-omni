...
"""Build a tiny local Qwen3-Omni Thinker checkpoint for multimodal generation smoke tests.

No tiny-random Qwen3-Omni checkpoint is published on the Hub, so multimodal e2e
tests build one locally. Following ``build_sd3_tiny_random.py``, everything is
...
ChatML BPE trained in process.

The chat template supports text, image, video, and audio message content so
``apply_chat_template`` and the Qwen3-Omni processor path can be exercised in
plumbing smoke tests. Tokenization quality is irrelevant for those tests.

Only the Thinker is exercised downstream (talker / code2wav are dropped at
FSDP-wrap time via ``Qwen3OmniThinkerAdapter.get_strip_modules``), so those
sub-configs are left at their defaults.

Usage:
    python tests/special_e2e/build_qwen3_omni_multimodal_tiny_random.py \
        --output-dir ~/models/tiny-random/Qwen3-Omni-Multimodal
"""
...
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/models/tiny-random/Qwen3-Omni-Multimodal")
...
def _build_tiny_config(vocab_size: int):
    ...
    # (also dropped at FSDP-wrap time via Qwen3OmniThinkerAdapter).
...
def _build_tiny_processor(tokenizer):
    ...
    vocab-independent, so their defaults are fine for a multimodal smoke test.
    """
...
def build(output_dir: str, *, seed: int = 42, dtype: torch.dtype = torch.bfloat16) -> str:
    ...
    for sub_config in (config.thinker_config, config.talker_config):
        for name, token_id in placeholder_token_ids.items():
            setattr(sub_config, name, token_id)
    ...
    _save_preprocessor_config(output_dir, processor)
    _write_chat_template_config(output_dir)
    return output_dir


def _save_preprocessor_config(output_dir: str, processor) -> None:
    """Save the flat feature/image preprocessor config used by Qwen3-Omni."""
    import json

    preprocessor_config_path = os.path.join(output_dir, "preprocessor_config.json")

    feature_dict = processor.feature_extractor.to_dict()
    image_dict = processor.image_processor.to_dict()
    feature_dict.pop("processor_class", None)
    image_dict.pop("processor_class", None)
    image_size = image_dict.pop("size", {})
    if "shortest_edge" in image_size:
        image_dict["min_pixels"] = image_size["shortest_edge"]
    if "longest_edge" in image_size:
        image_dict["max_pixels"] = image_size["longest_edge"]
    legacy_config = {
        **feature_dict,
        **image_dict,
    ...
    with open(preprocessor_config_path, "w") as f:
        json.dump(legacy_config, f, indent=2, sort_keys=True)


def _write_chat_template_config(output_dir: str) -> None:
    """Save chat_template.json for Qwen3-Omni adapter loading."""
    import json
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a tiny Qwen3-Omni multimodal Thinker checkpoint offline (random weights).",
    )
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Rebuild even when output-dir already contains config.json and chat_template.json",
    )
    args = parser.parse_args()

    dtype = {"float16": torch.float16, "bfloat16": torch.bfloat16, "float32": torch.float32}[args.dtype]
    output_dir = ensure_tiny_qwen3_omni_multimodal_checkpoint(
        args.output_dir,
        seed=args.seed,
        dtype=dtype,
        skip_if_exists=not args.force,
    )
    print(f"Tiny Qwen3-Omni multimodal checkpoint ready at {output_dir}")
