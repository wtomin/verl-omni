#!/usr/bin/env bash
# Smoke test for Qwen3-Omni offline MLLM DPO LoRA through the FSDP omni backend.
set -xeuo pipefail

NUM_GPUS=${NUM_GPUS:-2}
MODEL_PATH=${MODEL_PATH:-}
DATA_DIR=${DATA_DIR:-${HOME}/data/dummy_omni_preference_dpo_lora}
...
python3 -m verl_omni.trainer.main_omni \
    ...
    data.sampler.modality_ratios.image=1.0 \
    data.sampler.modality_ratios.video=1.0 \
    data.sampler.modality_ratios.audio=1.0 \
    ...
    trainer.experiment_name=qwen3-omni-entrypoint-fsdp-lora-dpo-smoke \
...
echo "Qwen3-Omni verl-omni FSDP LoRA DPO smoke test passed."
