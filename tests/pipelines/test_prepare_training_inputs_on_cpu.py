...
"""CPU tests for omni prepare_training_inputs dispatch."""
...
def prepare_training_inputs():
    return _bootstrap_pipeline_modules().prepare_training_inputs


class TestPrepareTrainingInputs:
    def test_extracts_standard_text_keys(self, prepare_training_inputs):
        ...
        model_inputs = prepare_training_inputs(_FakeModelConfig(), micro_batch)
        assert set(model_inputs) == {"input_ids", "attention_mask", "labels", "position_ids"}

    def test_squeezes_mrope_position_ids(self, prepare_training_inputs):
        micro_batch = TensorDict(
            {
    ...
    def test_drops_zero_multimodal_rows_and_casts_dtype(self, prepare_training_inputs):
        micro_batch = TensorDict(
            {
        ...
        model_inputs = prepare_training_inputs(_FakeModelConfig(), micro_batch, dtype=torch.bfloat16)
